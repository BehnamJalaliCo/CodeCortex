# Support

## Community support

Use GitHub Issues for reproducible bugs and feature requests. Include the CodeCortex version or commit, operating system, Python version, relevant command, expected behavior, and a minimal reproduction when possible.

Do not post credentials, private source code, access tokens, or vulnerability details in public issues.

## Security

Security vulnerabilities must follow `SECURITY.md` and should be reported through GitHub's private security-advisory flow.

## Commercial support

Commercial support, integration, deployment, and enterprise licensing may be offered separately from the Apache-2.0 community edition. See `COMMERCIAL.md` for the commercial model.
