...
> **Status: design target — not part of the currently shipped alpha surface yet.**
>
> These capabilities describe the next architecture direction. They are not released features until implementation, tests, benchmarks, documentation, and release gates confirm them.

The next step is a stronger **evidence-fusion architecture**.
...
## Evidence fusion is the destination
...
# Current vs next direction

| Capability | Current alpha | Next direction |
|---|---|---|
| Symbols | language-aware parsing | exact occurrence fusion when available |
| References | graph + semantic intelligence | exact/inferred provenance hierarchy |
| Dependencies | local imports + graph | resolved version + documentation evidence |
| Search | lexical + semantic + structural context | explicit AST-pattern search |
| Editing | guarded semantic symbol edits | preview-first structural migrations |
| Impact | graph walk + affected tests | evidence-quality-aware impact |
| Context | ranked + deduplicated + budgeted chunks | unified cross-provider evidence ranking |
| Confidence | explicit ambiguity | provenance + trust tiers |
| Offline behavior | local-first | remains local-first |
...
> **وضعیت: هدف طراحی — هنوز جزو سطح منتشرشده Alpha فعلی نیست.**
>
> این قابلیت‌ها فقط وقتی release محسوب می‌شوند که implementation، test، benchmark، documentation و release gate آن‌ها را ثابت کنند.

قدم بعدی معماری **Evidence Fusion** قوی‌تر است.
...
# وضعیت فعلی در برابر جهت بعدی

| قابلیت | Alpha فعلی | جهت بعدی |
|---|---|---|
| Symbol | parsing زبان‌آگاه | exact occurrence fusion |
| Reference | graph + semantic intelligence | hierarchy دقیق/استنباطی |
| Dependency | import + graph | resolved version + docs evidence |
| Search | lexical + semantic + structural context | AST-pattern search |
| Editing | guarded semantic edit | preview-first migration |
| Impact | graph walk + affected test | evidence-quality-aware impact |
| Context | ranked + dedup + budget | unified provider ranking |
| Confidence | ambiguity صریح | provenance + trust tier |
| Offline | local-first | همچنان local-first |
