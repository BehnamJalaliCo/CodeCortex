...
class Orchestrator:
    ...
    async def execute(self, request: AgentRequest) -> ExecutionResult:
        ...
        async with self.tracer.async_span("request.execute", trace_id=trace_id, attributes=attributes) as root_span:
    ...
    async def _healthy(self, capability: Capability, request: AgentRequest) -> bool:
        ...
        if engine is None:
            return False
        try:
            return bool(await asyncio.wait_for(engine.health(), timeout=self.policy.health_timeout_seconds))
        except asyncio.CancelledError:
            raise
        except Exception as exc:
            self.telemetry.emit("engine.health_failed", capability=capability.value, error_type=type(exc).__name__)
            ...
            try:
                async def execute_once() -> EngineResult:
                    if self.tracer and trace_id:
                        attrs: dict[str, object] = {"capability": capability.value, "attempt": attempt + 1}
                ...
                result = await asyncio.wait_for(execute_once(), timeout=self.policy.execution_timeout_seconds)
                latency_ms = (perf_counter() - started) * 1000
                self.telemetry.emit("engine.executed", capability=capability.value, duration_ms=latency_ms, attempts=attempt + 1)
                ...
                last_error = exc
                self.telemetry.emit("engine.failed", capability=capability.value, error_type=type(exc).__name__, attempt=attempt + 1)
                if attempt < retries:
                    continue
        latency_ms = (perf_counter() - started) * 1000
    ...
    async def _execute(self, request: AgentRequest, trace_id: str | None, parent_span: str | None) -> ExecutionResult:
        plan = self.router.route(request)
        self.telemetry.emit("route.created", kind=plan.request_kind.value, capabilities=[capability.value for capability in plan.selected])
        if self.tracer and trace_id:
            self.tracer.record("route.created", trace_id=trace_id, parent_id=parent_span, attributes={"kind": plan.request_kind.value, "capabilities": [item.value for item in plan.selected]})
        capabilities = [item for item in plan.selected if item != Capability.CONTEXT]
        outcomes = await asyncio.gather(*(self._run_engine(capability, request, trace_id, parent_span) for capability in capabilities))
        ...
        normalized_results = [result.model_copy(update={"chunks": [chunk for chunk in result.chunks if chunk.chunk_id in fitted_ids]}) for result in results]
        context_tokens = sum(chunk.tokens for chunk in fitted)
        self.telemetry.emit("context.fitted", budget=plan.context_budget, original=original_tokens, used=context_tokens, saved=max(0, original_tokens - context_tokens), chunks=len(fitted))
        return ExecutionResult(request=request, plan=plan, results=normalized_results, context_tokens=context_tokens, metadata={"original_context_tokens": original_tokens, "context_tokens_saved": max(0, original_tokens - context_tokens)})
