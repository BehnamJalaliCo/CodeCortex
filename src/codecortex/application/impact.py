...
class ImpactService:
    ...
    @staticmethod
    def _item(item: ImpactItem) -> dict[str, Any]:
        return {
    ...
    def analyze(self, query: str) -> dict[str, Any]:
        ...
        report = ImpactAnalyzer(graph).analyze(query)
        return {
            "target": report.target.model_dump(mode="json"),
            "risk_score": report.risk_score,
            "direct": [self._item(x) for x in report.direct],
            "indirect": [self._item(x) for x in report.indirect],
            "affected_tests": [self._item(x) for x in report.affected_tests],
        }
