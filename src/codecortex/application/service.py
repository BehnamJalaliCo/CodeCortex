...
class CortexApplicationService:
    ...
    async def repository_dashboard(self) -> dict[str, Any]:
        ...
        graph, stats = IncrementalGraphIndex(root).refresh()
        git = GitIntelligence(root).analyze(300)
        knowledge = ProjectKnowledgeExtractor(root).extract()
        counts = graph.counts()
        symbols = sum(v for k, v in counts.items() if k not in {"file", "module", "reference"})
        return {
            ...
            "index": {
                ...
                "duration_ms": stats.index.duration_ms,
            },
            "graph": {
            ...
            "git": {"commits": git.commits, "hot_files": [x.path for x in git.hot_files[:8]]},
            "runtime": {
                "health": await self.runtime.gateway.health(),
    ...
    def repository_files(self, *, limit: int = 500) -> dict[str, Any]:
        graph = self._graph()
        rows = [n for n in graph.nodes if n.kind == "file"][: min(5000, max(1, limit))]
        return {
            "files": [
                {"id": n.id, "name": n.name, "path": n.path, "language": n.metadata.get("language")}
                for n in rows
            ],
            "total": sum(n.kind == "file" for n in graph.nodes),
        }
    ...
    def repository_symbols(self, *, query: str = "", limit: int = 200) -> dict[str, Any]:
        ...
        rows = [n for n in candidates if n.kind not in {"file", "module", "reference"}][
            ...
            "symbols": [
                {
                    ...
                    "path": n.path,
                    "line": n.line,
                    "language": n.metadata.get("language"),
                    "container": n.metadata.get("container"),
                }
                for n in rows
            ],
            "total": sum(n.kind not in {"file", "module", "reference"} for n in graph.nodes),
        }

    def repository_graph(
        self, *, query: str = "", depth: int = 1, relation: str = "", limit: int = 400
    ) -> dict[str, Any]:
        graph = self._graph()
        bounded_depth = min(4, max(0, depth))
        bounded_limit = min(1000, max(1, limit))
        seed = graph.search(query, 8) if query.strip() else graph.nodes[: min(60, bounded_limit)]
        selected = {n.id for n in seed}
        eligible = [e for e in graph.edges if not relation or e.kind == relation]
        for _ in range(bounded_depth):
            ...
            if len(selected) >= bounded_limit:
                break
        nodes = [n for n in graph.nodes if n.id in selected][:bounded_limit]
        ids = {n.id for n in nodes}
        edges = [e for e in eligible if e.source in ids and e.target in ids][: bounded_limit * 4]
        return {
            "nodes": [
                {
                    "id": n.id,
