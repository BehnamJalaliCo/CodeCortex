...
def mount(app: Any, ctx: Any) -> None:
    ...
    def integrations(repository_id: str, _actor: str = Depends(ctx.principal)) -> dict[str, Any]:
        ...
        return {
            "integrations": [
                {"target": target.value, "detected": target in detected} for target in AgentTarget
            ]
        }

    @app.post(f"{ctx.prefix}/repositories/{{repository_id}}/integrations/preview")
    def preview(
        repository_id: str, payload: ConfigureRequest, _actor: str = Depends(ctx.principal)
    ) -> dict[str, Any]:
        _, configurator = config(repository_id)
        ...
        return {
            "mutations": [
                ...
                for item in configurator.configure(targets, dry_run=True)
            ]
        }

    @app.post(f"{ctx.prefix}/repositories/{{repository_id}}/integrations/apply")
    def apply(
        repository_id: str, payload: ConfigureRequest, actor: str = Depends(ctx.principal)
    ) -> dict[str, Any]:
        ...
        audit.record(
            ...
            metadata={"targets": [mutation.target.value for mutation in mutations]},
        )
        ctx.events.publish(
            "integrations.configured", {"repository_id": repository_id, "actor": actor}
        )
        return {
            "mutations": [
                asdict(mutation)
                | {
                    ...
                    "backup": str(mutation.backup) if mutation.backup else None,
                }
