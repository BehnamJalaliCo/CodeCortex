...
class ScaleRequest(BaseModel):
    targets: list[int] = Field(
        default_factory=lambda: [100000, 1000000], min_length=1, max_length=8
    )
...
def mount(app: Any, ctx: Any) -> None:
    ...
    def scale(
        repository_id: str, payload: ScaleRequest, actor: str = Depends(ctx.principal)
    ) -> dict[str, Any]:
        ...
        job = ctx.jobs.submit(
            "repository.scale",
            {"repository_id": repository_id, "targets": list(targets)},
            operation,
            actor=actor,
