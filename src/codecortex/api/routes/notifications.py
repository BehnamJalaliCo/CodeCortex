...
def mount(app: Any, ctx: Any) -> None:
    ...
    def notifications(
        include_acknowledged: bool = False, limit: int = 200, _actor: str = Depends(ctx.principal)
    ) -> dict[str, Any]:
        return {
            "notifications": [
                store.payload(item)
                for item in store.list(
                    include_acknowledged=include_acknowledged, limit=max(1, min(limit, 2000))
                )
            ]
        }
    ...
    def scan(actor: str = Depends(ctx.principal)) -> dict[str, int]:
        ...
        failed_jobs = [
            job
            for job in ctx.jobs.store.list(limit=500)
            if getattr(job.status, "value", job.status) == "failed"
        ]
        if failed_jobs:
            store.emit(
                "job.failure",
                ...
                f"{len(failed_jobs)} failed job(s) need attention.",
                "jobs",
        ...
        for repository in ctx.database.repositories():
            ...
            for key in runtime.active_backends:
                unhealthy.append(
                    (repository.repository_id, key)
                ) if not runtime.backend_manager.probe(
                    __import__("codecortex.backends.spec", fromlist=["BACKENDS"]).BACKENDS[key],
                    provision=False,
                ) else None
        if unhealthy:
            store.emit(
                ...
                f"{len(unhealthy)} active backend(s) are unhealthy.",
                "backends",
                {"backends": unhealthy},
            )
            created += 1
    ...
    def acknowledge(notification_id: str, actor: str = Depends(ctx.principal)) -> dict[str, bool]:
        if not store.acknowledge(notification_id):
            raise HTTPException(
                status_code=404, detail="notification not found or already acknowledged"
            )
        ctx.events.publish(
            "notification.acknowledged", {"notification_id": notification_id, "actor": actor}
        )
