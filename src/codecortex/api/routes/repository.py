...
def mount_repository_routes(app: Any, prefix: str, database: Any, runtimes: Any, principal: Any) -> None:
    ...
    def files(repository_id: str, limit: int = 500, _actor: str = Depends(principal)) -> dict[str, Any]:
        return service(repository_id).repository_files(limit=limit)

    @app.get(f"{prefix}/repositories/{{repository_id}}/symbols")
    def symbols(repository_id: str, query: str = "", limit: int = 200, _actor: str = Depends(principal)) -> dict[str, Any]:
