...
class MCPApplication:
    ...
    def tools(self) -> list[dict[str, Any]]:
        ...
        return [
            ...
            {"name": "cortex_memory_search", "description": "Search project memory and extracted knowledge.", "inputSchema": _schema({"query": text}, ["query"])},
    ...
    async def call(self, name: str, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        if name == "cortex_find_symbol":
            graph = self._graph()
            matches = [node for node in graph.search(str(arguments["query"]), 80) if node.kind not in {"file", "module", "reference"}]
        ...
        if name == "cortex_context":
            ...
            return {"chunks": [chunk.model_dump(mode="json") for chunk in prepared.chunks], "metrics": asdict(prepared.metrics)}
        ...
        if name == "cortex_validate":
            ...
            return {"validation": [item.model_dump(mode="json") for item in result.results if item.capability.value == "validation"]}
    ...
    async def dispatch(self, message: dict[str, Any]) -> dict[str, Any] | None:
        ...
        try:
            ...
            if method == "tools/call":
                ...
                payload = await self.application.call(str(params.get("name", "")), dict(params.get("arguments") or {}))
