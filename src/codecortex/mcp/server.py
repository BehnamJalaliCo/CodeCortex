...
class MCPApplication:
    ...
    def tools(self) -> list[dict[str, Any]]:
        ...
        return [
            ...
            },
            {
                ...
                "inputSchema": _schema(
                    {"query": text, "limit": {**positive_int, "default": 20}}, ["query"]
                ),
            },
            {
                "name": "cortex_context",
            ...
            },
            {
                ...
                "inputSchema": _schema(
                    {"base_ref": text, "head_ref": {"type": "string", "default": "HEAD"}},
                    ["base_ref"],
                ),
            },
            {
            ...
            },
            {
                ...
                "inputSchema": _schema(
                    {"query": text, "namespace": {"type": "string", "default": "project"}},
                    ["query"],
                ),
            },
            {
            ...
            },
            {
                ...
                "inputSchema": _schema(
                    {"query": text, "limit": {**positive_int, "default": 40}}, ["query"]
                ),
            },
            {
    ...
    async def call(self, name: str, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        if name == "cortex_find_symbol":
            matches = [
                node
                for node in self._graph().search(str(arguments["query"]), 80)
        ...
        if name == "cortex_architecture_drift":
            ...
            baseline = ArchitectureFingerprint.load(
                self.root / ".codecortex" / "architecture" / "baseline.json"
            )
            if baseline is None:
                return {
                    "baseline": "missing",
        ...
        if name == "cortex_symbol_history":
            return asdict(
                GitIntelligence(self.root).symbol_history(
                    str(arguments["path"]), int(arguments["start"]), int(arguments["end"])
                )
            )
        if name == "cortex_pr_intelligence":
            report = PRIntelligence(self.root, self._graph()).analyze(
                str(arguments["base_ref"]), str(arguments.get("head_ref", "HEAD"))
            )
            return {
        ...
        if name == "cortex_team_memory_search":
            ...
            return {
                "results": [
                    asdict(entry)
                    for entry in store.search_entries(
                        str(arguments.get("namespace", "project")), str(arguments["query"]), 20
                    )
                ]
            }
        ...
        if name == "cortex_trace_summary":
            return asdict(
                TaskTraceRecorder(self.root / ".codecortex" / "runtime" / "traces.jsonl").summarize(
    ...
    async def dispatch(self, message: dict[str, Any]) -> dict[str, Any] | None:
        ...
        try:
            ...
            if method == "tools/list":
                return self._result(
                    request_id, {"tools": self.application.tools(), "ttlMs": 300000}
                )
