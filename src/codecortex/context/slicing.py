...
class AstContextSlicer:
    ...
    def __init__(self, root: Path, token_counter: TokenCounter | None = None, *, context_lines: int = 2) -> None:
    ...
    def slice_symbol(self, path: Path, symbol: str, line: int | None = None, *, max_tokens: int = 800) -> str:
        source = self._read(path)
        if not source:
            return ""
        lines = source.splitlines()
        units = sorted(self.languages.parse(path, source), key=lambda item: (item.line, item.name))
        candidates = [unit for unit in units if unit.name == symbol]
        if candidates:
            unit = min(candidates, key=lambda item: abs(item.line - line)) if line is not None else candidates[0]
    ...
    def slice(self, query: str, path: Path, *, max_tokens: int = 1_200, limit: int = 6) -> list[ContextChunk]:
        source = self._read(path)
        if not source:
            return []
        ...
        for _, unit in scored[: max(1, limit)]:
            ...
            result.append(ContextChunk(source=f"ast:{relative}:{unit.name}", content=content, tokens=self.token_counter.count(content), relevance=0.96, metadata={"path": relative, "symbol": unit.name, "line": unit.line, "end_line": unit.end_line, "container": unit.container, "ast_slice": True}))
        return result
