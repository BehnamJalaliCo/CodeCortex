...
class RelationshipExtractor:
    ...
    _EXTENDS_RE = re.compile(r"\bclass\s+([A-Za-z_$]\w*)\s+extends\s+([A-Za-z_$]\w*)", re.MULTILINE)
    _IMPLEMENTS_RE = re.compile(r"\bclass\s+([A-Za-z_$]\w*)[^\n{]*\bimplements\s+([A-Za-z_$][\w$.,\s]*)", re.MULTILINE)
    _CALL_KEYWORDS = {"if", "for", "while", "switch", "catch", "return", "sizeof", "typeof", "function", "def", "class", "new"}
    ...
    def _python(source: str) -> list[Relationship]:
        ...
        class Visitor(ast.NodeVisitor):
            ...
            def visit_Call(self, node: ast.Call) -> None:
                ...
                if name:
                    result.append(Relationship("calls", name.split(".")[-1], node.lineno, containers[-1] if containers else None))
                self.generic_visit(node)
    ...
    def _imports(self, path: Path, source: str) -> list[Relationship]:
        ...
        if suffix in {".js", ".jsx", ".ts", ".tsx", ".mjs", ".cjs", ".mts", ".cts"}:
            regexes = [self._JS_IMPORT_RE]
        elif suffix == ".go":
            regexes = [self._GO_IMPORT_RE]
        elif suffix == ".rs":
            regexes = [self._RUST_USE_RE]
        elif suffix in {".java", ".cs"}:
            regexes = [self._JVM_IMPORT_RE]
        elif suffix in {".c", ".h", ".cc", ".cpp", ".cxx", ".hpp"}:
            regexes = [self._C_INCLUDE_RE]
        elif suffix == ".php":
            regexes = [self._PHP_USE_RE]
        elif suffix == ".rb":
            regexes = [self._RUBY_REQUIRE_RE]
        result: list[Relationship] = []
        for regex in regexes:
            for match in regex.finditer(source):
                result.append(Relationship("imports", match.group(1).strip(), source.count("\n", 0, match.start()) + 1))
        return result

    def _inheritance(self, source: str) -> list[Relationship]:
        result: list[Relationship] = []
        for match in self._EXTENDS_RE.finditer(source):
            result.append(Relationship("inherits", match.group(2), source.count("\n", 0, match.start()) + 1, match.group(1)))
        for match in self._IMPLEMENTS_RE.finditer(source):
            line = source.count("\n", 0, match.start()) + 1
            for target in match.group(2).split(","):
                name = target.strip()
                if name:
                    result.append(Relationship("implements", name, line, match.group(1)))
        return result
    ...
    def _calls(self, source: str) -> list[Relationship]:
        ...
        for match in self._CALL_RE.finditer(source):
            name = match.group(1)
            if name.lower() in self._CALL_KEYWORDS:
                continue
