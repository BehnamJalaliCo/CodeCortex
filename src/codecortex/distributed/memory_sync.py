"""Remote shared-memory synchronization with deterministic conflict resolution."""
...
class SharedMemoryReplica:
    """SQLite-backed replica that exchanges versioned mutations between nodes."""
    ...
    def _initialize(self) -> None:
        with self._connect() as connection:
            connection.executescript(
                ...
                CREATE TABLE IF NOT EXISTS shared_memory_changes (
                    sequence INTEGER PRIMARY KEY AUTOINCREMENT,
                    namespace TEXT NOT NULL,
                    key TEXT NOT NULL,
                    mutation TEXT NOT NULL
                );
                CREATE INDEX IF NOT EXISTS idx_shared_memory_changes_sequence
                    ON shared_memory_changes(sequence);
                """
            )
    ...
    def value(self, namespace: str, key: str) -> str | None:
        ...
        if mutation is None or mutation.tombstone:
            return None
    ...
    def _local_mutation(
        ...
        self._store(mutation, record_change=True)
        return mutation
    ...
    def export(self, after_sequence: int = 0, limit: int = 1000) -> list[tuple[int, MemoryMutation]]:
        with self._connect() as connection:
            rows = connection.execute(
                """
                SELECT sequence, mutation FROM shared_memory_changes
    ...
    def apply(self, mutations: list[MemoryMutation]) -> SyncResult:
        ...
        for incoming in mutations:
            current = self.get(incoming.namespace, incoming.key)
            if current is None:
                self._store(incoming, record_change=True)
                applied += 1
                continue
            ...
            if winner != current:
                self._store(winner, record_change=True)
                applied += 1
            else:
                ignored += 1
    ...
    def _resolve_concurrent(current: MemoryMutation, incoming: MemoryMutation) -> MemoryMutation:
        current_key = (current.updated_at, current.node_id)
        incoming_key = (incoming.updated_at, incoming.node_id)
    ...
    def _store(self, mutation: MemoryMutation, *, record_change: bool) -> None:
        encoded = json.dumps(mutation.to_dict(), sort_keys=True, separators=(",", ":"))
        with self._connect() as connection:
            connection.execute(
                """
                INSERT INTO shared_memory(namespace, key, value, node_id, clock, updated_at, tombstone)
                VALUES (?, ?, ?, ?, ?, ?, ?)
            ...
            if record_change:
                connection.execute(
                    "INSERT INTO shared_memory_changes(namespace, key, mutation) VALUES (?, ?, ?)",
