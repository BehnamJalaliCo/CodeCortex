...
app = typer.Typer(
    ...
    help="Run the authenticated CodeCortex remote MCP service.",
)
console = Console()
...
def serve(
    ...
    principal: Annotated[
        str,
        typer.Option("--principal", help="Authenticated principal name"),
    ] = "agent",
    ...
    if organization and workspace:
        ...
        def authorize(actor: str, tool: str) -> bool:
            return store.authorize_tool(
                organization,
                workspace,
                actor,
                tool,
    ...
    console.print(
        ...
        f"as node {node_id} for principal {principal}"
    )
    try:
