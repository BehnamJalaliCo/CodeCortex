...
class OrganizationPolicyStore:
    ...
    def create_organization(self, slug: str, display_name: str, *, owner: str) -> None:
        ...
        with self.db.connect() as connection:
            ...
            existing = connection.execute(
                "SELECT display_name FROM organizations WHERE slug = ?", (slug,)
            ).fetchone()
            if existing is not None:
                ...
                return
            connection.execute(
                "INSERT INTO organizations(slug, display_name, created_at) VALUES (?, ?, ?)",
                (slug, display_name, _now()),
            )
            connection.execute(
                "INSERT INTO organization_members(organization, principal, role) VALUES (?, ?, 'owner')",
                (slug, owner),
            )
    ...
    def set_member(self, organization: str, actor: str, principal: str, role: Role) -> None:
        ...
        self.audit.record(
            organization, actor, "member.set_role", principal, metadata={"role": role}
        )
    ...
    def require_role(self, organization: str, principal: str, minimum: Role) -> None:
        ...
        if role is None or _ROLE_RANK[role] < _ROLE_RANK[minimum]:
            self.audit.record(
                organization, principal, "authorization.denied", minimum, outcome="denied"
            )
            raise PermissionError(f"{principal} requires role {minimum} or higher")

    def create_workspace(
        self, organization: str, actor: str, workspace: str, *, project_root: str | None = None
    ) -> None:
        self.require_role(organization, actor, "admin")
        ...
        with self.db.connect() as connection:
            connection.execute(
                "INSERT INTO workspaces(organization, workspace, project_root, created_at) VALUES (?, ?, ?, ?) ON CONFLICT(organization, workspace) DO UPDATE SET project_root = excluded.project_root",
    ...
    def set_policy(
        ...
        *,
        ...
        policy = WorkspacePolicy(
            organization, workspace, tools, max_context_tokens, remote_access, metadata or {}
        )
        with self.db.connect() as connection:
            connection.execute(
                "INSERT INTO workspace_policies(organization, workspace, allowed_tools, max_context_tokens, remote_access, metadata) VALUES (?, ?, ?, ?, ?, ?) ON CONFLICT(organization, workspace) DO UPDATE SET allowed_tools = excluded.allowed_tools, max_context_tokens = excluded.max_context_tokens, remote_access = excluded.remote_access, metadata = excluded.metadata",
                (
                    organization,
                    workspace,
                    json.dumps(tools),
    ...
    def policy(self, organization: str, workspace: str) -> WorkspacePolicy | None:
        with self.db.connect() as connection:
            row = connection.execute(
                "SELECT * FROM workspace_policies WHERE organization = ? AND workspace = ?",
