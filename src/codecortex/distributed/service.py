...
class DistributedMCPApplication:
    def __init__(
        self, runtime: CortexRuntime, *, node_id: str, state_dir: Path | None = None
    ) -> None:
        ...
        return [
            *self.base.tools(),
            ...
            },
            {
                ...
                "description": "Register a worker. Remote identity is always the authenticated principal.",
                "inputSchema": {
                    "type": "object",
                    "properties": {
            ...
            },
            {
                ...
                "description": "Claim the next task compatible with the authenticated worker.",
                "inputSchema": {
                    "type": "object",
            ...
            },
            {
                ...
                "description": "Complete a leased task using its fencing token.",
                "inputSchema": {
                    "type": "object",
                    "properties": {
                        "node_id": {"type": "string", "minLength": 1},
                    ...
                    "required": ["task_id", "lease_token", "result"],
                    "additionalProperties": False,
                },
            },
        ]
    ...
    async def call(self, name: str, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        if name == "cortex_sync_push":
            ...
            result = self.memory.apply(
                [MemoryMutation.from_dict(item) for item in raw if isinstance(item, dict)]
            )
            return {
        ...
        if name == "cortex_worker_register":
            ...
            worker = self.workers.register_worker(
                node_id, tuple(str(item) for item in raw_capabilities), metadata
            )
            return {
        ...
        if name == "cortex_worker_complete":
            ...
            completed = self.workers.complete(
