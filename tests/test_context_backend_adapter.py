...
def test_context_backend_requests_mature_extras() -> None:
    spec = BACKENDS["context"]
    assert {"mcp", "code", "memory", "relevance"}.issubset(spec.extras)
    assert "headroom-ai[" in spec.source_requirement
...
def test_context_backend_uses_project_workspace(tmp_path: Path) -> None:
    ...
    assert client.env["HEADROOM_WORKSPACE_DIR"].startswith(str(tmp_path.resolve()))
