...
def test_fencing_token_rejects_stale_completion(tmp_path) -> None:
    ...
    with coordinator._connect() as connection:
        connection.execute(
            "UPDATE tasks SET lease_expires_at = ? WHERE task_id = 't'",
    ...
    with pytest.raises(PermissionError):
        asyncio.run(
            app.call_as(
                ...
                {"node_id": "worker-b", "capabilities": ["index"]},
            )
        )
    result = asyncio.run(
        app.call_as(
...
def test_organization_policy_is_in_remote_authorization_path(tmp_path) -> None:
    ...
    policy = RemoteAccessPolicy(
        ...
        authorizer=lambda principal, tool: store.authorize_tool(
            "acme", "repo", principal, tool, remote=True
        ),
    )
