...
def test_native_mcp_rejects_unknown_arguments(tmp_path) -> None:
    ...
    response = asyncio.run(server.dispatch({"jsonrpc": "2.0", "id": 1, "method": "tools/call", "params": {"name": "cortex_find_symbol", "arguments": {"query": "x", "unexpected": True}}}))
...
def test_initialize_negotiates_only_supported_protocols(tmp_path) -> None:
    server = MCPServer(MCPApplication(build_runtime(tmp_path)))
    ok = asyncio.run(server.dispatch({"jsonrpc": "2.0", "id": 1, "method": "initialize", "params": {"protocolVersion": "2025-06-18"}}))
    assert ok and ok["result"]["protocolVersion"] == "2025-06-18"
    bad = asyncio.run(server.dispatch({"jsonrpc": "2.0", "id": 2, "method": "initialize", "params": {"protocolVersion": "1900-01-01"}}))
...
def test_remote_mutations_require_explicit_opt_in() -> None:
    ...
    server = RemoteMCPServer(
        ...
        RemoteAccessPolicy(allowed_tools={"agent": frozenset({"read", "write"})}, mutating_tools=frozenset({"write"})),
    )
